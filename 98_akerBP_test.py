...
final_rs_mig = np.copy(rs_mig2)
final_rs_vel = np.copy(rs_vel3)[:cut_idx_to_mig]

plt.figure(figsize=(14,14))
plt.imshow(final_rs_mig, vmin = np.min(rs_mig2), vmax= -np.min(rs_mig2),
           cmap='seismic',extent=[0, new_ax_mig2, az_mig, 0])
plt.imshow(final_rs_vel, vmin = np.min(rs_vel), vmax= 2500,alpha=0.5,
