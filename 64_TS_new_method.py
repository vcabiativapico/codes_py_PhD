...
path_inv =gen_path + \
    '065_TS_analytique_less_deep/depth_demig_out/065_TS_analytique_deep_2024-08-27_10-01-00/results/depth_demig_output.csv'
...
# fl1 = '../input/50_ts_model/sm5_org.dat'
# fl2 = '../input/50_ts_model/sm5_ano.dat'
...
# # Model with smoothing 15 depth to a deeper spot
# path_ray = gen_path + \
#     '065_TS_analytique_less_deep/depth_demig_out/065_TS_analytique_deep_sm5_2024-08-27_09-57-19/rays/ray_0.csv'
# path_ray2 = gen_path + \
#     '065_TS_analytique_less_deep/depth_demig_out/065_TS_analytique_deep_ano_sm5_2024-08-27_09-57-02/rays/ray_0.csv'


# # # Model with smoothing 15 depth to a deeper spot
# path_ray = gen_path + \
#     '065_TS_analytique_less_deep/depth_demig_out/065_TS_analytique_deep_2024-08-27_10-01-00/rays/ray_0.csv'
# path_ray2 = gen_path + \
#     '065_TS_analytique_less_deep/depth_demig_out/065_TS_analytique_deep_ano_2024-08-27_10-01-03/rays/ray_0.csv'
...
tr_binv_fwi_org = gaussian_filter(tr_binv_fwi_org,8)
tr_binv_fwi_45 = gaussian_filter(tr_binv_fwi_45,8)
...
tr_badj_fwi_org = gaussian_filter(tr_badj_fwi_org,8)
tr_badj_fwi_45 = gaussian_filter(tr_badj_fwi_45,8)
...
tr_badj_fwi_org_norm = tr_badj_fwi_org/np.min(tr_badj_fwi_org[500:])
tr_badj_fwi_45_norm = tr_badj_fwi_45/np.min(tr_badj_fwi_45[500:])


def find_roots(tr,at):
    f = InterpolatedUnivariateSpline(at,tr,k=4)
    ct_point = f.derivative().roots()
    
    val_roots = f(ct_point)
    return ct_point,val_roots

at_point_conv_org, val_roots_conv_org = find_roots(convol_time_org_int[1200:],ray_time_int[1200:]) 
at_point_conv_ano, val_roots_conv_ano = find_roots(convol_time_ano_int[1200:],ray_time_int[1200:])

def extract_negatives(val_root,at_point):
    val_root_neg = []
    at_point_neg = []
    for i in range(val_root.size):
        if val_root[i] < 0:
            val_root_neg.append(val_root[i])
            at_point_neg.append(at_point[i])
    return np.array(at_point_neg),np.array(val_root_neg)
            

at_point_conv_org, val_roots_conv_org = extract_negatives(val_roots_conv_org,at_point_conv_org) 
at_point_conv_ano, val_roots_conv_ano= extract_negatives(val_roots_conv_ano,at_point_conv_ano) 

    
    
at_point_org_inv, val_roots_org_inv = find_roots(tr_binv_fwi_org_norm,at) 
at_point_ano_inv, val_roots_ano_inv = find_roots(tr_binv_fwi_45_norm,at)

at_point_org_adj, val_roots_org_adj = find_roots(tr_badj_fwi_org_norm,at) 
at_point_ano_adj, val_roots_ano_adj = find_roots(tr_badj_fwi_45_norm,at)


diff_at_roots_conv = at_point_conv_org - at_point_conv_ano
diff_at_roots_inv = at_point_org_inv - at_point_ano_inv
diff_at_roots_adj = at_point_org_adj - at_point_ano_adj
...
plt.plot(convol_time_org_int, ray_time_int, label='org')
plt.plot(convol_time_ano_int, ray_time_int, label='ano')
plt.plot(val_roots_conv_org,at_point_conv_org,'.')
plt.plot(val_roots_conv_ano,at_point_conv_ano,'.')
...
plt.figure(figsize=(7, 12))
plt.plot(tr_badj_fwi_org_norm, at, label='org')
plt.plot(tr_badj_fwi_45_norm, at, label='ano')
plt.plot(val_roots_org_adj,at_point_org_adj,'.')
plt.plot(val_roots_ano_adj,at_point_ano_adj,'.')
plt.title('modelled traces adj')
plt.ylim(1.95, ft-0.1)
plt.xlim(-2, 2)

# %matplotlib qt5
plt.figure(figsize=(7, 12))
plt.plot(diff_at_roots_inv*1000, at_point_org_inv-ft, label='mod qtv')
plt.plot(diff_at_roots_adj*1000, at_point_org_adj-ft, label='mod std')
plt.plot(diff_at_roots_conv*1000,at_point_conv_org-ft, label='mod rt')
plt.plot(ts_theo, ax_theo_corr, c='black', label='theo')
plt.legend()
