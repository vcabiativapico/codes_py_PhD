...
perc = 0.01
first_idx = find_first_index_greater_than(diff, np.max(diff)*perc)+240
...
pk_base = procs.extremum_func(org,win1=win1,win2=win2,maxi=False,si=dt)
pk_monitor = procs.extremum_func(ano,win1=win1,win2=win2,maxi=False,si=dt)
...
plt.plot(f_tau_opt(at), at, '-', color= 'tab:blue',label='Inv')
plt.plot(tau_opt, at_cut, 'o', color= 'tab:blue')
plt.axvline(-ts_picked/1000,color= 'tab:purple',label='max_cc')
# plt.plot(f_tau(at), at, '-', color= 'tab:green',label='True')
# plt.plot(tau_init, at_cut, '--', color= 'tab:orange',label='Init')
# plt.plot(tau_cut, at_cut, 'o',color= 'tab:green')
