...
gen_path = '/home/vcabiativapico/local/Demigration_SpotLight_Septembre2023/output/'
...
path_adj = gen_path + '044_fault_slope/depth_demig_out_fault2/STD/results/depth_demig_output.csv'
path_inv = gen_path + '044_fault_slope/depth_demig_out_fault2/QTV/results/depth_demig_output.csv'
...
base_path_ano = '../output/45_marm_ano_v3/fault_ano_114_percent/'
...
gather_path_fwi45 = '../output/45_marm_ano_v3/ano_fault'
...
diff = tr_badj_fwi_45 - tr_badj_fwi_org
plt.figure(figsize=(4,10))
plt.plot(diff,p_inv.at_)
plt.ylim(2.0,0)

plt.figure(figsize=(4,10))
plt.plot(tr_badj_fwi_org,p_inv.at_)
plt.plot(tr_badj_fwi_45,p_inv.at_)
plt.ylim(2.0,0)
plt.xlim(-0.5,0.5)
...
visualisation.overlay(tr_binv_fwi_org,tr_binv_fwi_45,si=p_inv.dt_,
                      ...
                      clist=['k', 'r'],
                      fontsize= 26)
plt.title('Base vs monitor 35')
plt.ylim(2050,0)
plt.xlim(-1,1)



binv_SLD_TS_fwi = procs.sliding_TS(tr_binv_fwi_org,tr_binv_fwi_45,oplen= 125,si=p_inv.dt_,taper= 15)
badj_SLD_TS_fwi = procs.sliding_TS(tr_badj_fwi_org,tr_badj_fwi_45,oplen= 125,si=p_inv.dt_,taper= 15)


ext_SLD_TS_badj = procs.extremum_func(badj_SLD_TS_fwi,win1=1500,win2=None,maxi=True,si=p_inv.dt_)
ext_SLD_TS_binv = procs.extremum_func(binv_SLD_TS_fwi,win1=1500,win2=None,maxi=True,si=p_inv.dt_)
...
binv_NRMS = procs.sliding_NRMS(tr_binv_fwi_org,tr_binv_fwi_45,si=dt)
binv_coeff_corr = procs.sliding_corrcoeff(tr_binv_fwi_org, tr_binv_fwi_45)

badj_NRMS = procs.sliding_NRMS(tr_badj_fwi_org,tr_badj_fwi_45,si=p_inv.dt_)
badj_coeff_corr = procs.sliding_corrcoeff(tr_badj_fwi_org, tr_badj_fwi_45)
...
visualisation.overlay(binv_NRMS,badj_NRMS,si=p_inv.dt_,
                      ...
                      fontsize = 26)
plt.title('NRMS')
mean_NRMS_badj = np.mean(badj_NRMS[1000:])
mean_NRMS_binv = np.mean(binv_NRMS[1000:])

max_NRMS_badj = np.max(badj_NRMS[1000:])
max_NRMS_binv = np.max(binv_NRMS[1000:])

min_NRMS_badj = np.min(badj_NRMS[1000:])
min_NRMS_binv = np.min(binv_NRMS[1000:])
...
mean_corr_coef_badj = np.mean(badj_coeff_corr[1000:])
mean_corr_coef_binv = np.mean(binv_coeff_corr[1000:])


max_corr_coeff_badj = np.max(badj_coeff_corr[1000:])
max_corr_coeff_binv = np.max(binv_coeff_corr[1000:])

min_corr_coeff_badj = np.min(badj_coeff_corr[1000:])
min_corr_coeff_binv = np.min(binv_coeff_corr[1000:])
...
mean_badj_SLD_TS = np.mean(badj_SLD_TS_fwi[1000:2000])
mean_binv_SLD_TS = np.mean(binv_SLD_TS_fwi[1000:2000])

max_badj_SLD_TS = np.max(badj_SLD_TS_fwi[1000:2000])
max_binv_SLD_TS = np.max(binv_SLD_TS_fwi[1000:2000])

min_badj_SLD_TS = np.min(badj_SLD_TS_fwi[1000:2000])
min_binv_SLD_TS = np.min(binv_SLD_TS_fwi[1000:2000])
...
path_demig = '/home/vcabiativapico/local/Demigration_SpotLight_Septembre2023/output/044_fault_slope'

path_results = '/result_ts_off/corr_results_both_angles_fault_114perc.csv'
...
file_slope_adj = path_demig + '/output/044_fault_slope/slope_STD_fault.csv'
file_slope_inv = path_demig + '/output/044_fault_slope/slope_QTV_fault.csv'
...
plt.rcParams['font.size'] = 16
plt.plot(p_hz_adj[0], p_hz_adj[1], label='Slope 35° STD')
plt.plot(p_hz_inv[0], p_hz_inv[1], label='Slope 37° QTV')
...
def plot_rays(p,hz,diff_ind_max,typ):
    ...
    # for i in range(92,101,5):
        ...
        # if 1==1:
            ...
            path_ray[i] = path_demig +'/output/044_fault_slope/depth_demig_out_fault/STD/rays/ray_'+str(i)+'.csv'
...
def plot_2rays(p1,p2,hz1,hz2,diff_ind_max,typ):
    ...
    # for i in range(92,101,5):
        ...
        # if 1==1:
            ...
            path_ray_inv[i] = path_demig +'/output/044_fault_slope/depth_demig_out_fault/STD/rays/ray_'+str(i)+'.csv'
            ...
            path_ray_adj[i] = path_demig +'/output/044_fault_slope/depth_demig_out_fault/STD/rays/ray_'+str(i)+'.csv'
            ...
            hfig = av1.imshow(Vit_model2.T[:,100:360],vmin = hmin,
                        vmax = hmax,aspect = 1, 
                        extent=(x_disc[100],x_disc[360],z_disc[-1],z_disc[0]),cmap='jet')
